				...
				for (int j = 0; j < nativeArray.Length; j++)
				{
					...
					if (m_PrefabObjectGeometryData.TryGetComponent(prefabRef.m_Prefab, out var componentData))
					{
						...
						if (!flag)
						{
							...
							if ((componentData.m_Flags & GeometryFlags.OccupyZone) != GeometryFlags.None)
							{
							...
							if ((componentData.m_Flags & GeometryFlags.HasLot) != GeometryFlags.None)
							{
						...
						if (!flag2)
						{
							...
							if (!flag || m_EditorMode)
							{
								...
								CollectionUtils.TryGet(nativeArray5, j, out var value);
								boundsMask |= CommonUtils.GetBoundsMask(Game.Net.SearchSystem.GetLayers(value, default(Game.Net.UtilityLane), layers, ref m_PrefabRefData, ref m_PrefabNetData, ref m_PrefabNetGeometryData));
							}
						}
						if (!flag4)
						{
						...
						m_SearchTree.Add(item2, new QuadTreeBoundsXZ(bounds, boundsMask, componentData.m_MinLod));
					}
					else
					{
						...
						m_SearchTree.Add(item2, new QuadTreeBoundsXZ(bounds2, boundsMask2, lod));
					}
				}
				return;
			}
			...
			for (int k = 0; k < nativeArray.Length; k++)
			{
				...
				if (m_PrefabObjectGeometryData.TryGetComponent(prefabRef2.m_Prefab, out var componentData3))
				{
					...
					if (!flag5)
					{
						...
						if ((componentData3.m_Flags & GeometryFlags.HasLot) != GeometryFlags.None)
						{
					...
					if (!flag6)
					{
						...
						if (!flag5 || m_EditorMode)
						{
							...
							CollectionUtils.TryGet(nativeArray9, k, out var value2);
							boundsMask3 |= CommonUtils.GetBoundsMask(Game.Net.SearchSystem.GetLayers(value2, default(Game.Net.UtilityLane), layers2, ref m_PrefabRefData, ref m_PrefabNetData, ref m_PrefabNetGeometryData));
						}
					}
					if (!flag8)
					{
					...
					m_SearchTree.Update(item3, new QuadTreeBoundsXZ(bounds3, boundsMask3, componentData3.m_MinLod));
				}
				else
				{
					...
					m_SearchTree.Update(item3, new QuadTreeBoundsXZ(bounds4, boundsMask4, lod2));
				}
			}
		}

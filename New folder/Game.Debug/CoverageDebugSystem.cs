...
public class CoverageDebugSystem : BaseDebugSystem
{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			for (int i = 0; i < bufferAccessor.Length; i++)
			{
				...
				if (dynamicBuffer.Length != 0)
				{
					...
					ServiceCoverage serviceCoverage = dynamicBuffer[(int)m_Service];
					Color color = Color.Lerp(Color.red, Color.green, serviceCoverage.m_Coverage.x * 0.25f);
					Color b = Color.Lerp(Color.red, Color.green, serviceCoverage.m_Coverage.y * 0.25f);
					...
					Line3.Segment segment = new Line3.Segment(@float, @float + new float3(0f, serviceCoverage.m_Coverage.x * 10f, 0f));
					...
					for (int j = 1; j <= num; j++)
					{
						...
						float num3 = math.lerp(serviceCoverage.m_Coverage.x, serviceCoverage.m_Coverage.y, float2.y);
					...
					for (int k = 1; k <= num2; k++)
					{
						...
						float num4 = math.lerp(serviceCoverage.m_Coverage.x, serviceCoverage.m_Coverage.y, float3.y);
	...
	private struct TypeHandle
	{
		...
		public void __AssignHandles(ref SystemState state)
		{
			...
			__Game_Net_ServiceCoverage_RO_BufferTypeHandle = state.GetBufferTypeHandle<ServiceCoverage>(isReadOnly: true);
		}
	}

...
public class PreCullingSystem : GameSystemBase, IPostDeserialize
{
	...
	private struct TreeCullingJob1 : IJobParallelFor
	{
		...
		public void Execute(int index)
		{
			...
			{
			case 0:
				m_StaticObjectSearchTree.Iterate(ref iterator, 3, m_NodeBuffer.GetSubArray(0, num), m_SubDataBuffer.GetSubArray(0, num));
				break;
			case 1:
				m_NetSearchTree.Iterate(ref iterator, 3, m_NodeBuffer.GetSubArray(num, num), m_SubDataBuffer.GetSubArray(num, num));
				break;
			case 2:
				m_LaneSearchTree.Iterate(ref iterator, 3, m_NodeBuffer.GetSubArray(num * 2, num), m_SubDataBuffer.GetSubArray(num * 2, num));
				break;
			}
		}
	}
	...
	private struct TreeCullingJob2 : IJobParallelFor
	{
		...
		public void Execute(int index)
		{
			...
			{
			case 0:
				m_StaticObjectSearchTree.Iterate(ref iterator, m_SubDataBuffer[index], m_NodeBuffer[index]);
				break;
			case 1:
				m_NetSearchTree.Iterate(ref iterator, m_SubDataBuffer[index], m_NodeBuffer[index]);
				break;
			case 2:
				m_LaneSearchTree.Iterate(ref iterator, m_SubDataBuffer[index], m_NodeBuffer[index]);
				break;
			}
		}
	}
	...
	private struct TreeCullingIterator : INativeQuadTreeIteratorWithSubData<Entity, QuadTreeBoundsXZ, int>, IUnsafeQuadTreeIteratorWithSubData<Entity, QuadTreeBoundsXZ, int>
	{
		...
		public void Iterate(QuadTreeBoundsXZ bounds, int subData, Entity entity)
		{
			...
			if (flag != flag2)
			{
				CullingAction value = new CullingAction
				{
					m_Entity = entity,
					m_UpdateFrame = -1
				};
				if (flag)
				{
					value.m_Flags = ActionFlags.PassedCulling;
				}
				else if ((m_VisibleMask & bounds.m_Mask) != 0)
				{
					value.m_Flags = ActionFlags.CrossFade;
				}
	...
	public struct InitializeCullingJob : IJobChunk
	{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			if (bufferAccessor.Length != 0)
			{
				...
				{
					ref CullingInfo reference = ref nativeArray.ElementAt(i);
					if (m_UpdateAll || flag)
					{
						...
						if (m_PrefabObjectGeometryData.HasComponent(prefabRef.m_Prefab))
						{
							...
							if (!flag2 || m_EditorMode)
							{
								...
								reference.m_Mask |= CommonUtils.GetBoundsMask(Game.Net.SearchSystem.GetLayers(value, default(Game.Net.UtilityLane), layers, ref m_PrefabRefData, ref m_PrefabNetData, ref m_PrefabNetGeometryData));
							}
			...
			if (nativeArray4.Length != 0)
			{
				...
				{
					ref CullingInfo reference2 = ref nativeArray.ElementAt(j);
					if (m_UpdateAll || flag)
					{
						...
						if (m_PrefabObjectGeometryData.TryGetComponent(prefabRef2.m_Prefab, out var componentData))
						{
							...
							if (!flag4 && (!flag3 || m_EditorMode))
							{
								...
								CollectionUtils.TryGet(nativeArray9, j, out var value2);
								reference2.m_Mask |= CommonUtils.GetBoundsMask(Game.Net.SearchSystem.GetLayers(value2, default(Game.Net.UtilityLane), layers2, ref m_PrefabRefData, ref m_PrefabNetData, ref m_PrefabNetGeometryData));
							}
			...
			else if (nativeArray5.Length != 0)
			{
				...
				{
					ref CullingInfo reference3 = ref nativeArray.ElementAt(k);
					if (m_UpdateAll || flag)
					{
			...
			else if (nativeArray6.Length != 0)
			{
				...
				{
					ref CullingInfo reference4 = ref nativeArray.ElementAt(l);
					if (m_UpdateAll || flag)
					{
			...
			else if (nativeArray7.Length != 0)
			{
				...
				{
					ref CullingInfo reference5 = ref nativeArray.ElementAt(m);
					if (m_UpdateAll || flag)
					{
						...
						if (m_PrefabLaneGeometryData.HasComponent(prefabRef3.m_Prefab))
						{
							...
							if (!flag7 && curve.m_Length > 0.1f)
							{
								...
								CollectionUtils.TryGet(nativeArray17, m, out var value3);
								CollectionUtils.TryGet(nativeArray18, m, out var value4);
								reference5.m_Mask |= CommonUtils.GetBoundsMask(Game.Net.SearchSystem.GetLayers(value3, value4, defaultLayers, ref m_PrefabRefData, ref m_PrefabNetData, ref m_PrefabNetGeometryData));
							}
							int num = netLaneGeometryData.m_MinLod;
							if (m_PrefabUtilityLaneData.TryGetComponent(prefabRef3.m_Prefab, out var componentData7) && (componentData7.m_UtilityTypes & m_DilatedUtilityTypes) != UtilityTypes.None)
							{
			...
			else
			{
				...
				{
					ref CullingInfo reference6 = ref nativeArray.ElementAt(n);
					if (m_UpdateAll || flag)
					{
	...
	private struct QueryCullingJob : IJobChunk
	{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			if (bufferAccessor.Length != 0)
			{
				...
				{
					ref CullingInfo reference = ref nativeArray2.ElementAt(i);
			...
			for (int j = 0; j < nativeArray2.Length; j++)
			{
				ref CullingInfo reference2 = ref nativeArray2.ElementAt(j);
	...
	private struct QueryRemoveJob : IJobChunk
	{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			if (chunk.Has(ref m_TransformFrameType))
			{
				...
				for (int i = 0; i < nativeArray2.Length; i++)
				{
					if (nativeArray2.ElementAt(i).m_CullingIndex != 0)
					{
			...
			for (int j = 0; j < nativeArray2.Length; j++)
			{
				if (nativeArray2.ElementAt(j).m_CullingIndex != 0)
				{

...
public class PoliceEmergencyDispatchSystem : GameSystemBase
{
	...
	private struct PoliceDispatchJob : IJobChunk
	{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			if (index == m_NextUpdateFrameIndex && !chunk.Has(ref m_DispatchedType) && !chunk.Has(ref m_PathInformationType))
			{
				...
				NativeArray<PoliceEmergencyRequest> nativeArray2 = chunk.GetNativeArray(ref m_PoliceEmergencyRequestType);
				...
				for (int i = 0; i < nativeArray2.Length; i++)
				{
					...
					{
						if (!ValidateReversed(entity, policeEmergencyRequest.m_Site))
						{
						...
						{
							FindVehicleTarget(unfilteredChunkIndex, entity, policeEmergencyRequest.m_Site);
						}
					}
					else
					{
						if (!ValidateSite(entity, policeEmergencyRequest.m_Site))
						{
							m_CommandBuffer.DestroyEntity(unfilteredChunkIndex, entity);
							continue;
						}
						if (SimulationUtils.TickServiceRequest(ref serviceRequest))
						{
							FindVehicleSource(unfilteredChunkIndex, entity, policeEmergencyRequest.m_Site, policeEmergencyRequest.m_Target, policeEmergencyRequest.m_Purpose);
						}
					}
			...
			NativeArray<PoliceEmergencyRequest> nativeArray5 = chunk.GetNativeArray(ref m_PoliceEmergencyRequestType);
			...
			if (nativeArray4.Length != 0)
			{
				...
				for (int j = 0; j < nativeArray4.Length; j++)
				{
					...
					{
						if (!ValidateSite(entity2, policeEmergencyRequest2.m_Site))
						{
			...
			for (int k = 0; k < nativeArray5.Length; k++)
			{
				...
				{
					if (!ValidateReversed(entity3, policeEmergencyRequest3.m_Site))
					{
				...
				{
					if (!ValidateSite(entity3, policeEmergencyRequest3.m_Site))
					{
						m_CommandBuffer.DestroyEntity(unfilteredChunkIndex, entity3);
						continue;
					}
		...
		private bool ValidateReversed(Entity entity, Entity source)
		{
			...
			{
				if ((componentData.m_Flags & (PoliceStationFlags.HasAvailablePatrolCars | PoliceStationFlags.HasAvailablePoliceHelicopters)) == 0 || (componentData.m_PurposeMask & (PolicePurpose.Emergency | PolicePurpose.Intelligence)) == 0)
				{
					return false;
				}
				if (componentData.m_TargetRequest != entity)
				{
					if (m_PoliceEmergencyRequestData.HasComponent(componentData.m_TargetRequest))
					{
						return false;
					}
			...
			{
				if ((componentData2.m_State & (PoliceCarFlags.ShiftEnded | PoliceCarFlags.Disabled)) != 0 || componentData2.m_RequestCount > 1 || (componentData2.m_PurposeMask & (PolicePurpose.Emergency | PolicePurpose.Intelligence)) == 0 || m_ParkedCarData.HasComponent(source))
				{
					return false;
				}
				if (componentData2.m_TargetRequest != entity)
				{
					if (m_PoliceEmergencyRequestData.HasComponent(componentData2.m_TargetRequest))
					{
						return false;
					}
		...
		private void FindVehicleTarget(int jobIndex, Entity requestEntity, Entity vehicleSource)
		{
			...
			if (m_PoliceCarData.TryGetComponent(vehicleSource, out var componentData) && (componentData.m_State & PoliceCarFlags.Returning) == 0)
			{
	...
	private struct TypeHandle
	{
		...
		public void __AssignHandles(ref SystemState state)
		{
			...
			__Game_Simulation_PoliceEmergencyRequest_RO_ComponentTypeHandle = state.GetComponentTypeHandle<PoliceEmergencyRequest>(isReadOnly: true);
	...
	protected override void OnCreate()
	{
		...
		m_RequestQuery = GetEntityQuery(ComponentType.ReadOnly<PoliceEmergencyRequest>(), ComponentType.ReadOnly<UpdateFrame>());
	...
	protected override void OnUpdate()
	{
		...
		PoliceDispatchJob jobData = new PoliceDispatchJob
		{
			...
			m_PoliceEmergencyRequestType = InternalCompilerInterface.GetComponentTypeHandle(ref __TypeHandle.__Game_Simulation_PoliceEmergencyRequest_RO_ComponentTypeHandle, ref base.CheckedStateRef),

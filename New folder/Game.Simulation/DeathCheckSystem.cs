	...
	private struct DeathCheckJob : IJobChunk
	{
		...
		private void Die(ArchetypeChunk chunk, int chunkIndex, int i, Entity citizen, Entity household, NativeArray<Game.Citizens.Student> students, NativeArray<HealthProblem> healthProblems)
		{
			...
			else
			{
				...
				if ((value.m_Flags & HealthProblemFlags.RequireTransport) != HealthProblemFlags.None)
				{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			for (int i = 0; i < chunk.Count; i++)
			{
				...
				float time = 1f * ageInDays / (float)m_TimeSettings.m_DaysPerYear / (float)kMaxAgeInGameYear;
				if (citizen.GetPseudoRandom(CitizenPseudoRandom.Death).NextFloat() < m_HealthcareParameterData.m_DeathRate.Evaluate(time))
				{
				...
				else
				{
					...
					int num = 10 - citizen.m_Health / 10;
					int num2 = num * num;
					num2 += 8;
					if (random.NextInt(kUpdatesPerDay * 1000) <= num2)
					{
						Die(chunk, unfilteredChunkIndex, i, entity, household, nativeArray4, nativeArray2);
						continue;
					}
					float num3 = MathUtils.Logistic(3f, 1000f, 6f, (float)num / 10f - 0.35f);
					int num4 = 0;
					if (m_CurrentBuildings.HasComponent(entity))
					{
						...
						{
							num4 = m_HospitalData[currentBuilding].m_TreatmentBonus;
						}
					}
					num3 -= (float)(10 * num4);
					CityUtils.ApplyModifier(ref num3, modifiers, CityModifierType.RecoveryFailChange);
					if (random.NextFloat(1000f) >= num3)
					{
						if ((value.m_Flags & HealthProblemFlags.RequireTransport) != HealthProblemFlags.None)
						{

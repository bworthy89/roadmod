...
public class SchoolAISystem : GameSystemBase
{
	...
	private struct SchoolTickJob : IJobChunk
	{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			NativeArray<ServiceUsage> nativeArray3 = chunk.GetNativeArray(ref m_ServiceUsageType);
			...
			for (int i = 0; i < chunk.Count; i++)
			{
				...
				for (int num4 = dynamicBuffer.Length - 1; num4 >= 0; num4--)
				{
					if (m_Students.TryGetComponent(dynamicBuffer[num4], out var componentData2) && m_Citizens.TryGetComponent(dynamicBuffer[num4], out var componentData3))
					{
						...
						else
						{
							...
							float graduationProbability = GraduationSystem.GetGraduationProbability(componentData2.m_Level, componentData3.m_WellBeing, componentData, modifiers, 0.5f, efficiency);
							if (graduationProbability > 0.001f)
							{
								...
								float num6 = math.saturate(GraduationSystem.GetDropoutProbability(componentData2.m_Level, componentData2.m_LastCommuteTime, fee, 0, ageInDays, 0.5f, failedEducationCount, graduationProbability, ref m_EconomyParameters));
				...
				{
					float graduationProbability2 = GraduationSystem.GetGraduationProbability(componentData.m_EducationLevel, 50, componentData, modifiers, 0.5f, efficiency);
				...
				if (nativeArray3.Length != 0)
				{
					...
					{
						m_Usage = 1f * (float)num3 / (float)componentData.m_StudentCapacity
					};
				}
			}
		}

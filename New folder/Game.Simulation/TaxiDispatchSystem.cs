...
public class TaxiDispatchSystem : GameSystemBase
{
	...
	private struct TaxiDispatchJob : IJobChunk
	{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			if (index == m_NextUpdateFrameIndex && !chunk.Has(ref m_DispatchedType) && !chunk.Has(ref m_PathInformationType))
			{
				...
				NativeArray<TaxiRequest> nativeArray2 = chunk.GetNativeArray(ref m_TaxiRequestType);
				...
				for (int i = 0; i < nativeArray2.Length; i++)
				{
					...
					{
						if (!ValidateReversed(entity, taxiRequest.m_Seeker))
						{
						...
						{
							FindVehicleTarget(unfilteredChunkIndex, entity, taxiRequest.m_Seeker);
						}
					}
					else
					{
						if (!ValidateTarget(entity, taxiRequest.m_Seeker, dispatched: false))
						{
							m_CommandBuffer.DestroyEntity(unfilteredChunkIndex, entity);
							continue;
						}
						if (SimulationUtils.TickServiceRequest(ref serviceRequest))
						{
							FindVehicleSource(unfilteredChunkIndex, entity, taxiRequest.m_Seeker, taxiRequest.m_Type, taxiRequest.m_Priority);
						}
					}
			...
			NativeArray<TaxiRequest> nativeArray5 = chunk.GetNativeArray(ref m_TaxiRequestType);
			...
			if (nativeArray4.Length != 0)
			{
				...
				for (int j = 0; j < nativeArray4.Length; j++)
				{
					...
					{
						if (!ValidateTarget(entity2, taxiRequest2.m_Seeker, dispatched: true))
						{
			...
			for (int k = 0; k < nativeArray5.Length; k++)
			{
				...
				{
					if (!ValidateReversed(entity3, taxiRequest3.m_Seeker))
					{
				...
				{
					if (!ValidateTarget(entity3, taxiRequest3.m_Seeker, dispatched: false))
					{
						m_CommandBuffer.DestroyEntity(unfilteredChunkIndex, entity3);
						continue;
					}
		...
		private bool ValidateReversed(Entity entity, Entity source)
		{
			if (m_TransportDepotData.TryGetComponent(source, out var componentData))
			{
				...
				{
					if (m_TaxiRequestData.HasComponent(componentData.m_TargetRequest))
					{
						return false;
					}
			...
			if (m_TaxiData.TryGetComponent(source, out var componentData2))
			{
				if ((componentData2.m_State & (TaxiFlags.Requested | TaxiFlags.RequiresMaintenance | TaxiFlags.Dispatched | TaxiFlags.Disabled)) != 0 || m_ParkedCarData.HasComponent(source))
				{
					return false;
				}
				if (m_CurrentRouteData.TryGetComponent(source, out var componentData3) && m_BoardingVehicleData.HasComponent(componentData3.m_Route))
				{
					return false;
				}
				if (componentData2.m_TargetRequest != entity)
				{
					if (m_TaxiRequestData.HasComponent(componentData2.m_TargetRequest))
					{
						return false;
					}
					componentData2.m_TargetRequest = entity;
					m_TaxiData[source] = componentData2;
				}
				return true;
			}
			return false;
		...
		private bool ValidateTarget(Entity entity, Entity target, bool dispatched)
		{
			...
			{
				TaxiStand value = m_TaxiStandData[target];
				...
				{
					if (m_TaxiRequestData.HasComponent(value.m_TaxiRequest))
					{
						return false;
					}
					value.m_TaxiRequest = entity;
					m_TaxiStandData[target] = value;
				}
				return true;
			}
			if (m_RideNeederData.HasComponent(target))
			{
				RideNeeder value2 = m_RideNeederData[target];
				if (value2.m_RideRequest != entity)
				{
					if (m_TaxiRequestData.HasComponent(value2.m_RideRequest))
					{
						return false;
					}
					value2.m_RideRequest = entity;
					m_RideNeederData[target] = value2;
				}
				return true;
			}
			return false;
		...
		private void FindVehicleTarget(int jobIndex, Entity requestEntity, Entity vehicleSource)
		{
			...
			if (m_TaxiData.TryGetComponent(vehicleSource, out var componentData) && (componentData.m_State & TaxiFlags.Transporting) != 0)
			{
	...
	private struct TypeHandle
	{
		...
		public void __AssignHandles(ref SystemState state)
		{
			...
			__Game_Simulation_TaxiRequest_RO_ComponentTypeHandle = state.GetComponentTypeHandle<TaxiRequest>(isReadOnly: true);
			...
			__Game_Routes_TaxiStand_RW_ComponentLookup = state.GetComponentLookup<TaxiStand>();
			__Game_Buildings_TransportDepot_RW_ComponentLookup = state.GetComponentLookup<TransportDepot>();
			__Game_Vehicles_Taxi_RW_ComponentLookup = state.GetComponentLookup<Taxi>();
			__Game_Creatures_RideNeeder_RW_ComponentLookup = state.GetComponentLookup<RideNeeder>();
